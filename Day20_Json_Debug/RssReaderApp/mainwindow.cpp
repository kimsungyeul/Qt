...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
    ...
    /* RSS 피드 다운로드와 웹페이지 표시를 위한 프로그래스바 */
...
MainWindow::~MainWindow( )
{
    ...
    SaveUrls* saveUrl = (SaveUrls*)lib.resolve("SaveUrls");     /* new SaveUrls() */
    QStringList urlList;
...
void MainWindow::replyFinished(QNetworkReply *netReply)
{
    ...
    if(!vt.isNull()){
        qDebug() << "Redirectied to:" << vt.toUrl().toString();
        ...
        if(!doc.setContent(str, false, &error)){
            ...
            for(int i=0; i<nodeList.length();i++) {
                ...
                model->itemFromIndex(index)->setFlags(Qt::ItemIsSelectable |
                                                      Qt::ItemIsEnabled);
            }
        }
    }
